...
public class MessageController {
    ...
    @Operation(summary = "프로젝트 AI 메시지 전송 및 제안 생성")
    @PostMapping
    public ApiResponse<MessageResponse> createMessage(
        ...
        return ApiResponse.success(messageService.createMessage(projectId, userText));
    }

    @Operation(summary = "프로젝트 AI 변경 제안 승인 및 적용")
    @PostMapping("/{messageId}/apply")
    public ApiResponse<MessageApplyResponse> applyMessage(
            @PathVariable Long projectId,
            @PathVariable Long messageId
    ) {
        return ApiResponse.success(messageApplyService.apply(projectId, messageId));
    }
}
